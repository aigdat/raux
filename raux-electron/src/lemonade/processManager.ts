...
class LemonadeProcessManager {
	...
	async startLemonade(envOverrides: Record<string, string> = {}) {
		try {
			...
			const serverOptions = {
				...
				onExit: (code: number | null) => {
					...
					if (code !== 0 && this.restartAttempts < LemonadeProcessManager.MAX_RESTART_ATTEMPTS) {
						...
						logInfo(
							`[LemonadeProcessManager] Attempting restart ${this.restartAttempts}/${LemonadeProcessManager.MAX_RESTART_ATTEMPTS}`
						);
						setTimeout(
							() => this.startLemonade(envOverrides),
							LemonadeProcessManager.RESTART_DELAY_MS
						);
					}
				}
			};
			...
			logError(
				`[LemonadeProcessManager] Error starting Lemonade: ${err && err.toString ? err.toString() : String(err)}`
			);
			return false;
		}
	}
