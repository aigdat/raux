...
import { lemonadeProcessManager } from './lemonade/processManager';
...
import { lemonadeStatusMonitor } from './lemonade/statusMonitor';
...
const routeApplicationFlow = async (): Promise<void> => {
	try {
		...
		logError(
			'Error in routing application flow: ' + (err && err.toString ? err.toString() : String(err))
		);
		throw err;
	}
};
